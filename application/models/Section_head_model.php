...
class Section_head_model extends CI_Model
{
	function index(){
...
public function my_totice($limit=null,$start=null)
{
    ...
    ->where(['t1.is_deleted'=>'NOT_DELETED','t1.notice_type'=>'Section_Head','t1.status'=>'1','t1.to_date >='=>date('Y-m-d')]) ;             
    if (@$_POST['search']) {
...
public function count_row_notice($tb)
{
 $this->db->select('*')
 ->from($tb)
 ->where(['is_deleted'=>'NOT_DELETED','status'=>'1','notice_type'=>'Section_Head','to_date >='=>date('Y-m-d')]);
...
public function count_row_hw()
{
 ...
 ->join('v_sec_student t5','t1.sec_id=t5.sec_id')
 ->distinct('t5.sec_id')
...
public function search_data($tea_id,$searchTerm) {
   ...
   if (@$searchTerm) {
    ...
    // $this->db->or_like('stu_id', $searchTerm);
    // $this->db->or_like('enrollment', $searchTerm);
    // $this->db->or_like('father_no', $searchTerm);
    // $this->db->or_like('mobile', $searchTerm);
    // $this->db->or_like('father', $searchTerm);
    // $this->db->or_like('mother', $searchTerm);
    // $query = $this->db->get('v_sec_student');
    // $this->db->where(['is_deleted'=>'NOT_DELETED','regstatus'=>'1','IsLeft'=>'0']);
    // return $query->result();
    }else
    {
        return [];
...
public function view_period($tea_id,$search,$limit=null,$start=null)
{
    ...
    ->join('section_master t2','t1.section_id=t2.id','left')
    ->join('section_head_mapping t3','t3.section_id=t1.section_id','left')
    ->distinct('t3.section_id')
    ->where(['t1.is_deleted'=>'NOT_DELETED','t3.tea_id'=>$tea_id]);      
    if (@$_POST['search']) {
        $data['search'] = $_POST['search'];
        $this->db->group_start();
        $this->db->like('t2.section_name',$_POST['search']);
...
public function getStudentLeave($limit=null,$start=null)
{
    ...
    ->where(['t1.is_deleted'=>'NOT_DELETED','t2.regstatus'=>'1','t2.IsLeft'=>'0','leave_date'=>date('Y-m-d')])
    ->order_by('t2.fname');             
    if (@$_POST['search']) {
        $data['search'] = $_POST['search'];
        $this->db->group_start();
