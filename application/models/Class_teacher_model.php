...
public function my_totice($limit=null,$start=null)
{
    ...
    ->where(['t1.is_deleted'=>'NOT_DELETED','t1.notice_type'=>'CLass_Teacher','t1.status'=>'1','t1.to_date >='=>date('Y-m-d')]) ;             
    if (@$_POST['search']) {
...
public function count_row_notice($tb)
{
 $this->db->select('*')
 ->from($tb)
 ->where(['is_deleted'=>'NOT_DELETED','status'=>'1','notice_type'=>'Class_Teacher','to_date >='=>date('Y-m-d')]);
...
public function find_section($id)
{
    ...
    $this->db->join('class_teacher_mapping t2' , 't1.id=t2.section_id');
...
public function getStudentLeave($limit=null,$start=null)
{
    ...
    ->where(['t1.is_deleted'=>'NOT_DELETED','t2.regstatus'=>'1','t2.IsLeft'=>'0','t1.leave_date'=>date('Y-m-d')])
    ->order_by('t2.fname');             
    if (@$_POST['search']) {
        $data['search'] = $_POST['search'];
        $this->db->group_start();
