...
<script>
  $(document).on("submit", '.ajaxsubmit5', function(event) {
    ...
    var $this = $(this);
    var form_data = new FormData(this); // Correct usage of FormData

    if ($this.hasClass("append")) {
        var append_data = $($this.attr('append-data')).val();
        form_data.append('append', append_data); // Append data to FormData object
    }

    var form_valid = true;

    if ($this.hasClass("validate-form")) {
        if (!$this.valid()) { // Corrected usage of validation
            form_valid = false;
        }
    }
    ...
    if (form_valid) {
        $.ajax({
            url: '<?=base_url();?>Accounts/CheckSrNO',
            type: 'POST',
            data: form_data, 
            contentType: false, // Ensure these settings for FormData
            processData: false,
            success: function(data) {
                if (data == 'success') {
                    ...
                    $('#submitButton').prop('disabled', false).html('Submit');
                    alert_toastr("error", "Sorry Please enter correct SR No");
                }
            }
        });
    } else {
