<div class="card-body table-responsive p-0 pb-2">
                ...
                <?php $i=$page;foreach($rows as $r){?>
                  <tr>
                    ...
                    <td><?=$r->reason;?></td>
                    <td><?php if($r->approval_status=='APPROVED'){echo '<button class="btn btn-sm btn-success">APPROVED</button>';}elseif($r->approval_status=='PENDING'){echo '<button class="btn btn-sm btn-warning" href="javascript:void(0)" data-toggle="modal" data-target="#showModal" data-whatever="Update  ( Exam  :-'.$r->fname.' '.$r->lname.')" data-url="'.$reply_url.'/'.$r->id.'" title="Update">PENDING</button>';}elseif($r->approval_status=='REJECTED'){echo '<button class="btn btn-sm btn-danger">REJECTED</button>';} ;?></td>
