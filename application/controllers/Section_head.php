    ...
    public function teacher_student_leave_request($action=null,$p1=null,$p2=null,$p3=null)
{
    ...
    $data['search']           = $this->input->post('search');
    $this->load->view('erp/section_head/header',$data);
    $this->load->view('erp/section_head/student_leave_request',$data);
    $this->load->view('erp/section_head/footer');
    break;
    case 'tb':
        ...
        $this->load->library('pagination');
        $config = array();
        $config["base_url"]     = base_url()."section-head-student-marks-upload/tb";
        $config["total_rows"]   = count($this->section_head_model->getStudentLeave());
        $data['total_rows']     = $config["total_rows"];
        $config["per_page"]     = 10;
        $config["uri_segment"]  = 2;
        $config['attributes']   = array('class' => 'pag-link ');
        ...
        $data['page']           = $page = ($p1!=null) ? $p1 : 0;
        $data['search']         = $this->input->post('search');
        $data['action_url']         = base_url().'section-head-student-leave-request/save';
        $data['reply_url']         = base_url().'section-head-student-leave-request/open_model';
        $data['rows']           = $this->section_head_model->getStudentLeave($config["per_page"],$page);
        $page                       = 'erp/section_head/tb_student_leave';
        $this->load->view($page, $data); 
        break;
        case 'open_model':
        $data['action_url']         = base_url().'class-teacher-student-leave-request/save/'.$p1;
        $page           = 'erp/section_head/student_leave_reply';
        $data['row']    = $this->section_head_model->getRow('student_leave',['id'=>$p1]);
        $data['form_id']            = uniqid();
        $this->load->view($page, $data);
        break;
        case 'save':
   ...
   public function sectionhead_student_timetable($action=null,$p1=null,$p2=null,$p3=null)
   { 
       switch ($action) {
           case 'view':
       $data['menu_id'] = $this->uri->segment(2);
       ...
       $data['search']           = $this->input->post('search');
       $this->load->view('erp/section_head/header',$data);
       $this->load->view('erp/section_head/section_timetable_master',$data);
       $this->load->view('erp/section_head/footer');
       break;
       case 'tb':
           $id=$_SESSION['MUserId'];
           $data['section']=$p1;
           $data['section_name']   = $this->section_head_model->view_data_id('section_master',$p1);
           $data['period']         = $this->section_head_model->view_period_data('section_periods');
           $page                       = 'erp/section_head/tb_timetable_section';
           $this->load->view($page, $data); 
           break;
