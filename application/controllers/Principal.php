...
class Principal extends Inst {
    ...
    public function profile($action=null,$p1=null,$p2=null,$p3=null)
    {
        ...
        $this->load->view('erp/principal/header',$data);
        $this->load->view('erp/principal/my_profile',$data);
        $this->load->view('erp/principal/footer');
        break;
        case 'edit-image':
            $return['res'] = 'error';
  ...
  public function teacher_student_leave_request($action=null,$p1=null,$p2=null,$p3=null)
{
    ...
    $data['search']           = $this->input->post('search');
    $this->load->view('erp/principal/header',$data);
    $this->load->view('erp/principal/student_leave_request',$data);
    $this->load->view('erp/principal/footer');
    break;
    case 'tb':
        $userid=$id=$_SESSION['MUserId'];
        ...
        $config = array();
        $config["base_url"]     = base_url()."principal-student-marks-upload/tb";
        $config["total_rows"]   = count($this->principal_model->getStudentLeave());
        $data['total_rows']     = $config["total_rows"];
        $config["per_page"]     = 10;
        $config["uri_segment"]  = 2;
        $config['attributes']   = array('class' => 'pag-link ');
        ...
        $data['page']           = $page = ($p1!=null) ? $p1 : 0;
        $data['search']         = $this->input->post('search');
        $data['action_url']         = base_url().'principal-student-leave-request/save';
        $data['reply_url']         = base_url().'principal-student-leave-request/open_model';
        $data['rows']           = $this->principal_model->getStudentLeave($config["per_page"],$page);
        $page                       = 'erp/principal/tb_student_leave';
        $this->load->view($page, $data); 
        break;
        case 'open_model':
        $data['action_url']         = base_url().'principal-student-leave-request/save/'.$p1;
        $page           = 'erp/principal/student_leave_reply';
        $data['row']    = $this->principal_model->getRow('student_leave',['id'=>$p1]);
        $data['form_id']            = uniqid();
        $this->load->view($page, $data);
        break;
        case 'save':
    ...
    break;
     ...
     public function principal_student_timetable($action=null,$p1=null,$p2=null,$p3=null)
   { 
       switch ($action) {
           case 'view':
       $data['menu_id'] = $this->uri->segment(2);
       ...
       $data['search']           = $this->input->post('search');
       $this->load->view('erp/principal/header',$data);
       $this->load->view('erp/principal/section_timetable_master',$data);
       $this->load->view('erp/principal/footer');
       break;
       case 'tb':
           $id=$_SESSION['MUserId'];
           $data['section']=$p1;
           $data['section_name']   = $this->principal_model->view_data_id('section_master',$p1);
           $data['period']         = $this->principal_model->view_period_data('section_periods');
           $page                       = 'erp/principal/tb_timetable_section';
           $this->load->view($page, $data); 
           break;
