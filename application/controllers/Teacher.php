...
class Teacher extends CI_Controller {
    ...
    public function teacher_dash()
    {
        ...
        $data['total_exam']     = $this->teacher_model->count_row('exam_master');
        $data['roles'] = $this->erp_model->view_role($id);
   ...
   public function student_attendance_register($action=null,$p1=null,$p2=null,$p3=null)
    ...
    public function teacher_student_leave_request($action=null,$p1=null,$p2=null,$p3=null)
    {
        ...
        case 'tb':
            ...
            $data['reply_url']         = base_url().'teacher-student-leave-request/open_model';
            $data['rows']           = $this->teacher_model->getStudentLeave($config["per_page"],$page);
            $page                       = 'erp/teacher/tb_student_leave';
            $this->load->view($page, $data); 
            break;
            case 'open_model':
            $data['action_url']         = base_url().'teacher-student-leave-request/save/'.$p1;
            $page           = 'erp/teacher/student_leave_reply';
            $data['row']    = $this->teacher_model->getRow('student_leave',['id'=>$p1]);
            $data['form_id']            = uniqid();
            $this->load->view($page, $data);
            break;
            case 'save':
    ...
    public function teacher_student_timetable($action=null,$p1=null,$p2=null,$p3=null)
   { 
       switch ($action) {
           case 'view':
       $data['menu_id'] = $this->uri->segment(2);
       ...
       $data['search']           = $this->input->post('search');
       $this->load->view('erp/teacher/header',$data);
       $this->load->view('erp/teacher/section_timetable_master',$data);
       $this->load->view('erp/teacher/footer');
       break;
       case 'tb':
           $id=$_SESSION['MUserId'];
           ...
           $page                       = 'erp/teacher/tb_timetable_section';
           $this->load->view($page, $data); 
           break;
